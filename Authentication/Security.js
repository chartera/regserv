...
var fs = require('fs');
    ...
    tls = require('tls'),
    option = {
        key: fs.readFileSync(__dirname +'/registrationService.pem'),        // private key
        cert: fs.readFileSync(__dirname +'/registrationService-cert.pem'),  // public key
        ca: [],               // clients mi denen du kommunizieren kanns
        requestCert: true
    };
*/
...
var // larger numbers mean better security, less
    config = {
 ...
 */
var _hashPassword = function (password, callback) {
 ...
 */
var _verifyPassword = function (password, combined, callback) {
    ...
    try{
        ...
        console.error("[AuthenticationService] _verifyPassword -> " + e);
        callback(e, null)
    }

};

Security.prototype.hashPassword = function (password, callback) {
    _hashPassword(password, function (err, bin) {
        if(!err) {
...
Security.prototype.verifyPassword = function (password, combined, callback) {
    _verifyPassword(password, new Buffer(combined, "base64"), function (err, result) {
        if(!err){
