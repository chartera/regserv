...
if(singleMode  === "true"){
    console.log("Orchestrator in singleMode", singleMode)
}else{
    ...
    const
        ...
        log = log4js.getLogger('Orchestrator.main on pid: ' + process.pid);

    if (cluster.isMaster) {

        let
            clients = {},
            handshake   = zmq.socket("rep").bind('ipc://orchestrator-rep.ipc'),      // message for initialize back channel
            router_in   = zmq.socket('pull').bind('ipc://orchestrator-puller.ipc'), // message for jobs
            // router_in   = zmq.socket('pull').bind('tcp://127.0.0.1:55555'), // message for jobs

            toWorker    = zmq.socket('push').bind('ipc://_orchestrator-pusher.ipc'),   // send jobs to Worker
            fromWorker  = zmq.socket('pull').bind('ipc://_orchestrator-puller.ipc'); // receive job response
        ...
        let
            fromRouter = zmq.socket('pull').connect("ipc://_orchestrator-pusher.ipc"),
            ...
            map = require('./handler')({
                ...
                context: {
                    name: "context",
                    handshake: "ipc://context-rep.ipc",
                    api: "ipc://context-puller.ipc",
                    timeout: 95000
                },
                authorization: {
                    name: "authorization",
                    handshake: "ipc://authorization-rep.ipc",
                    api: "ipc://authorization-puller.ipc",
                    timeout: 95000,
                ...
                authentication: {
                    ...
                    api: "ipc://authentication-puller.ipc",
                    timeout: 95000
                }
            }),
            clients = {},
        ...
        fromRouter.on('message', function(message) {
            try{
                ...
                if(map.has(cmd)) {
                    ...
                    error(header, "Orchestrator found no cmd!");
                }
            }catch(e){
                log.error(e);
            }
